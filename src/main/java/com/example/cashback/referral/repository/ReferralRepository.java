...
public interface ReferralRepository extends JpaRepository<Referral, UUID> {
    ...
    @Query("SELECT r.referrer.id, COUNT(r) FROM Referral r GROUP BY r.referrer.id")
    List<Object[]> countReferralsPerReferrer();

    // Count successful referrals per referrer
    @Query("SELECT r.referrer.id, COUNT(r) FROM Referral r WHERE r.status = 'SUCCESS' GROUP BY r.referrer.id")
    List<Object[]> countSuccessfulReferralsPerReferrer();

    // Find top N referrers by number of referrals
    @Query("SELECT r.referrer.id, COUNT(r) FROM Referral r GROUP BY r.referrer.id ORDER BY COUNT(r) DESC")
